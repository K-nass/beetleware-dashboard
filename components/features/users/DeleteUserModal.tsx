...
export default function DeleteUserModal({ userId }: DeleteUserModalProps) {
  ...
  const [state, formAction, isPending] = useActionState<ActionResponse<void> | null, FormData>(deleteUser, null);

  useEffect(() => {
    getUserById(userId)
      .then((res) => {
        if (res.success && res.data) setUser(res.data);
        else setLoadError(res.error ?? "Failed to load user");
      })
