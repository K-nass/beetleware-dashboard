...
export default function AddFaqModalRoute() {
  ...
  const [formData, setFormData] = useState<AddFaqCommand>({ questionEn: "", questionAr: "", answerEn: "", answerAr: "" });
  ...
  const validate = (): boolean => {
    ...
    if (!formData.questionAr.trim()) newErrors.questionAr = "Arabic question is required";
    if (!formData.answerEn.trim()) newErrors.answerEn = "English answer is required";
    if (!formData.answerAr.trim()) newErrors.answerAr = "Arabic answer is required";
    setErrors(newErrors);
            ...
            <div className="p-6 overflow-y-auto flex-1">
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="flex justify-end gap-3 pt-4 border-t border-gray-200">
                  ...
                  <button type="submit" disabled={isSubmitting} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50">{isSubmitting ? "Creating..." : "Create FAQ"}</button>
                </div>
              </form>
            </div>
          </>
