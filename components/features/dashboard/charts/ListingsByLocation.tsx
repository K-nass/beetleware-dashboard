...
export default function ListingsByLocation() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const response = await api.get('/dashboard/charts/listings-by-location');
        
        if (response.data.succeeded) {
          const chartData: ChartData[] = response.data.data.value.map((item: LocationData) => ({
  ...
  const maxValue = Math.max(...data.map(item => item.listings));
